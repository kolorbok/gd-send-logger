...
static std::string requestURL() {
    ...
    return apiBase() + "/requests?mode=" + mode +
        ...
        "&minSend=" + ((g_filters.sendTypes.empty() && g_filters.sendSources.empty()) ? g_filters.minSend : "any") +
        "&sendType=" + joinCSV(g_filters.sendTypes) +
...
protected:
    ...
    void addDarkRoundedRect(CCPoint position, CCSize size, float radius, GLubyte opacity, int z) {
        ...
        shape->drawPolygon(points.data(), static_cast<unsigned int>(points.size()), ccc4f(0.f,0.f,0.f,static_cast<float>(opacity)/255.f),0.f,ccc4f(0.f,0.f,0.f,0.f));
        shape->setPosition(position);
        m_mainLayer->addChild(shape,z);
    }
    void addPanel(CCPoint position, CCSize size) { addDarkRoundedRect(position,size,8.f,40,-1); }
    void addText(char const* text, CCPoint position, float scale=.36f, char const* font="goldFont.fnt") {
        auto* label=CCLabelBMFont::create(text,font); if(!label) return; label->setScale(scale); label->setPosition(position); m_mainLayer->addChild(label,2);
    }
    void addField(CCPoint position,float width=112.f,float height=30.f){addDarkRoundedRect(position,{width,height},5.f,82,0);}
    CCLabelBMFont* addValue(CCPoint position){auto* label=CCLabelBMFont::create("","bigFont.fnt");if(!label)return nullptr;label->setScale(.40f);label->setPosition(position);m_mainLayer->addChild(label,2);return label;}
    void addArrow(CCPoint position,bool right,SEL_MenuHandler handler){auto* spr=CCSprite::createWithSpriteFrameName("GJ_arrow_01_001.png");if(!spr)return;spr->setScale(.52f);spr->setFlipX(right);auto* btn=CCMenuItemSpriteExtra::create(spr,this,handler);btn->setPosition(position);btn->setSizeMult(1.f);m_buttonMenu->addChild(btn,3);}
    void addCycleRow(char const* title,CCLabelBMFont*& value,float centerX,float y,SEL_MenuHandler left,SEL_MenuHandler right){constexpr float fieldW=102.f;constexpr float fieldH=28.f;constexpr float gap=14.f;addText(title,{centerX,y+22.f},.36f);addField({centerX,y},fieldW,fieldH);value=addValue({centerX,y});addArrow({centerX-fieldW/2.f-gap,y},false,left);addArrow({centerX+fieldW/2.f+gap,y},true,right);}
    void normalizeDependentFilters(){if(m_working.status=="my_unchecked"||m_working.status=="team_unchecked"||m_working.status=="rejected")m_working.minSend="any";}
    void refresh(){
        ...
        if(m_sendType){std::string text;if(m_working.sendTypes.empty()&&m_working.sendSources.empty())text="Any";else if(m_working.sendTypes.empty())text=std::to_string(m_working.sendSources.size())+" SOURCES";else if(m_working.sendTypes.size()==1)text=prettyMinSend(m_working.sendTypes.front());else text=std::to_string(m_working.sendTypes.size())+" TYPES";m_sendType->setString(text.c_str());m_sendType->setScale(text.size()>13?.32f:.40f);}
    ...
    bool initFor(){
        ...
        float height=m_staff?270.f:220.f; constexpr float width=400.f;
        if(!Popup::init(width,height))return false;
        setTitle("REQUEST FILTERS","goldFont.fnt",.60f,15.f);
        addPanel({width/2.f,height/2.f-4.f},{356.f,m_staff?188.f:144.f});
        constexpr float leftX=115.f,rightX=285.f;float topY=m_staff?214.f:162.f;
        addText("DIFFICULTY",{leftX,topY+22.f},.36f);addField({leftX,topY},102.f,28.f);
        ...
        float y=topY-46.f;
        if(m_staff){addCycleRow("MY STATUS",m_status,leftX,y,menu_selector(RequestFiltersPopup::statusPrev),menu_selector(RequestFiltersPopup::statusNext));
            addText("SEND TYPE",{rightX,y+22.f},.36f);addField({rightX,y},102.f,28.f);
            m_sendType=CCLabelBMFont::create("Any","bigFont.fnt");if(m_sendType){m_sendType->setScale(.40f);m_sendTypeButton=CCMenuItemSpriteExtra::create(m_sendType,this,menu_selector(RequestFiltersPopup::onSendTypePicker));}
        ...
        auto* resetSpr=ButtonSprite::create("RESET",104,true,"bigFont.fnt","GJ_button_04.png",30.f,.56f);auto* resetBtn=CCMenuItemSpriteExtra::create(resetSpr,this,menu_selector(RequestFiltersPopup::onReset));resetBtn->setPosition({112.f,31.f});resetBtn->setSizeMult(1.f);m_buttonMenu->addChild(resetBtn,3);
        auto* applySpr=ButtonSprite::create("APPLY",104,true,"bigFont.fnt","GJ_button_01.png",30.f,.56f);auto* applyBtn=CCMenuItemSpriteExtra::create(applySpr,this,menu_selector(RequestFiltersPopup::onApply));applyBtn->setPosition({288.f,31.f});applyBtn->setSizeMult(1.f);m_buttonMenu->addChild(applyBtn,3);
        refresh();return true;
    }
    void onDifficultyPicker(CCObject*){if(auto* popup=DifficultyPickerPopup::create(m_working.difficulties,[this](auto const& values){this->m_working.difficulties=values;this->refresh();}))popup->show();}
    void onSendTypePicker(CCObject*){if(auto* popup=SendTypePickerPopup::create(m_working.sendTypes,m_working.sendSources,[this](auto const& types,auto const& sources){this->m_working.sendTypes=types;this->m_working.sendSources=sources;this->refresh();}))popup->show();}
    ...
    void applyLoadedState() {
        ...
        if (g_requestList.empty()) {
            if (g_filters.status == "unchecked") {
                setStatus("0 shown for MY UNCHECKED - server filter returned no rows");
            } else {
